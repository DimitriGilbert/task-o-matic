...
export const enhanceCommand = new Command("enhance")
  ...
  .action(wrapCommandHandler("Task enhancement", async (options: EnhanceCommandOptions) => {
    ...
    const enhanceSingleTask = async (taskId: string) => {
      if (options.dry) {
        ...
        console.log(chalk.blue(`[DRY RUN] Would enhance: ${task?.title || taskId}`));
        return;
      }
    ...
    if (options.taskId) {
      ...
      await executeBulkOperation(
        (taskId) => enhanceSingleTask(taskId),
        {
